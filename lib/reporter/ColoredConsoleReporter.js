...
export default class ColoredConsoleReporter extends ConsoleReporter {
  ...
  endReport() {
    ...
    this.print(`  ${colors[this.theme.task.result.ok.color]("OK")} ${rep.ok.length}`);
    this.print(` ${colors[this.theme.task.result.failed.color]("Failed")} ${rep.failed.length}`);
    this.print(` ${colors[this.theme.task.result.ignored.color]("Ignored")} ${rep.ignored.length}`);
